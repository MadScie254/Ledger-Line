...
export function ApAgingWorkspace() {
  const { data, isLoading, error } = useQuery<ApAgingReport>({
    queryKey: ["ap-aging"],
    queryFn: () => fetch("/api/reports/ap-aging").then((res) => res.json()),
  });

  if (isLoading) {
    return (
      <div className="flex h-full items-center justify-center p-8 text-slate-500">
        Loading A/P aging report...
      </div>
    );
  }
  ...
  if (error || !data) {
    return (
      <div className="flex h-full flex-col items-center justify-center p-8 text-rust-700">
        <p className="font-semibold">Failed to load A/P aging</p>
  ...
  return (
    <div className="flex h-full flex-col space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-ink-900 font-serif">A/P Aging Summary</h1>
        <p className="text-sm text-slate-500">Accounts payable by aging bucket</p>
      </div>
      ...
      <div className="flex items-center justify-between rounded-[10px] border border-slate-200 bg-paper-50 px-4 py-3">
        <span className="font-semibold text-slate-700">Total Outstanding Payables</span>
      ...
      <div className="flex-1 rounded-[10px] border border-slate-200 bg-white shadow-sm overflow-hidden flex flex-col">
        {data.rows.length === 0 ? (
          <div className="flex flex-1 flex-col items-center justify-center p-12 text-center">
            <p className="text-lg font-semibold text-ink-900">No outstanding payables!</p>
            <p className="mt-1 text-sm text-slate-500">All bills are paid or cleared.</p>
          </div>
        ) : (
          <DataTable
            data={data.rows}
            ...
            columns={[
              {
                key: "vendor",
                header: "Vendor",
                cell: (row) => <span className="font-medium text-ink-900">{row.vendorName}</span>,
              },
              {
                key: "billNo",
                header: "Bill #",
                cell: (row) => <span className="font-mono text-slate-600">{row.billNo}</span>,
              },
              {
                key: "billDate",
                header: "Bill Date",
                cell: (row) => <span className="text-slate-600">{format(new Date(row.billDate), "MMM d, yyyy")}</span>,
              },
              {
                key: "dueDate",
                header: "Due Date",
